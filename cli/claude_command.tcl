...
`--dialogue` drops the machinery - tool calls, their results, thinking - and leaves the conversation. A `:roles` suffix of `user` or `assistant` keeps one side of it alone. The replies outweigh the prompts by an order of magnitude. A session too large to read whole is read as its prompts first: they say what the human wanted and where the work turned. The anchors are then the way back in for the few replies worth the tokens.
